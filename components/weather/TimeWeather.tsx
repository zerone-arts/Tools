...
export default function TimeWeather({
  ...
  return (
    <div className="bg-gray-200 w-[250px] h-[230px] rounded-xl flex-grow flex-col b  dark:bg-zinc-900 flex p-2 justify-center">
      <div className="w-full overflow-scroll flex flex-col gap-[6px] ">
        <div className="pl-2">
          <h1 className="font-semibold">Today</h1>
        </div>
        <ul className="flex gap-[10px] h-[100px] w-full pl-2  ">
          {WeathersToday.map((item, idx) => {
            return (
              <li
                key={idx}
                className="bg-white w-[70px] rounded-lg flex flex-col justify-center items-center gap-1 dark:bg-zinc-700 flex-shrink-0"
              >
